    ...
    <>
      {PROJECTS.map((project) => (
         <Link to={`/work/${project.id}`}
            key={project.id}
            className="w-full relative max-w-xl mb-3 p-5 md:p-20 bg-stripes h-72 items-center flex rounded-md"
         >
            ...
            <img src={project.image} className="h-auto w-full p-1 mt-4 ring-1 ring-neutral-300 bg-white rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.12)]" alt={project.title || 'Project image'} />
         </Link>
      ))}
    </>
  );
